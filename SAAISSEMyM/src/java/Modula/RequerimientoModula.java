    ...
    public void enviaRequerimiento(String F_IdFact) {
        ...
        try {
            ...
            try {
                ...
                ResultSet rset = con.consulta("select F_ClaCli, F_FecEnt, F_IdFact from v_folioremisiones where F_IdFact = '" + F_IdFact + "' and  F_StsFact=0 group by F_IdFact");
                while (rset.next()) {
                ...
                rset = con.consulta("select F_ClaCli, F_FecEnt, F_IdFact, F_ClaPro, F_ClaLot, F_FecCad, F_Cb F_Cant, F_Id from v_folioremisiones where F_IdFact = '" + F_IdFact + "' and F_StsFact=0");
                while (rset.next()) {
                    /*
                     * La 'A' es de inserción
                     */
                    conModula.ejecutar("insert into IMP_ORDINI_RIGHE values('" + rset.getString("F_FolRemi") + "','','" + rset.getString("F_ClaPro") + "','" + rset.getString("F_ClaLot") + "','1','" + rset.getString("F_Cant") + "','" + rset.getString("F_CB") + "','" + df4.format(df3.parse(rset.getString("F_FecCad"))) + "','')");
